...
1. Add `nix-config/hostSpecs/[hostname].nix`, `nix-config/hosts/nixos/[hostname]/` and `nix-config/home/[user]/[hostname].nix` files. You must declare the configuration settings for the target host as usual in your nix-config.
...
- update the `.sops.yaml` file with:
    - public age keys entries for the host
    - update the `creation_rules` for `shared.yaml` with the host age key for the target host.
...
There was an automation stage to re-key the secrets and push these changes to nix secrets but I got rid of it. We'll talk more about the manual steps below.
...
Your existing config should already have a `hardware-configuration.nix` and a functioning complement of sops secrets and sops creation rules. Therefore, many of the steps presented by the script can be safely skipped. The applicable steps will be noted below.
...
- Disk size: a decent _minimum_ disk size without swap is 25GB to accommodate for multiple generations on a testing machine.
     ...
     For example, if you need 50GB of storage for you machine and you also specified a swapsize of 8GB in nix-config, then allocate 58GB for the VMs disk size.
...
If you need WiFi in the live environment:
...
1. "Run nixos-anywhere installation?" default yes - This initiates installation of the minimal-config environment.
    ...
    4. If required, turn WiFi back on:
        ```bash
...
2. "Generate host (ssh-based) age key?" default yes - usually only needed for new hosts
    1. If you do any of this make sure to go over to `nix-secrets` and ensure the re-key has been done, changes are pushed, and you've updated your flake in `nixos`:
        ```bash
        cd ../nix-secrets
        cat .sops.yaml # Make sure host keys were properly added
        sops updatekeys sops/*.yaml # Make sure secrets are encrypted with the new keys
        git add . && git commit -m "chore: rekey" && git push
        cd ../nixos
        task update # update the flake to the new nix-secrets
        ```
3. "Do you want to copy your full nix-config and nix-secrets to $target_hostname?" default yes - copies the source of both repos from the source machine to the target machine, faster than cloning from github/lab
    1. "Do you want to rebuild immediately?" default yes - builds the full config
4. "Do you want to commit and push the generated hardware-configuration.nix for $target_hostname to nix-config?" default yes - This will _only_ be presented if you said yes to question 1.2 and will push the file to your repo with an appropriate commit message.
...
If you did any of the steps from 3 through 5, you will need to rebuild for the changes to take effect. Run `task rebuild:[hostname]` from the `nix-config` directory on the new host.
