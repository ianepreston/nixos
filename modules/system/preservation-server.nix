    ...
    preservation = {
      ...
      preserveAt."/persist" = {
        directories = [
          ...
          # ----- Observability -----
          # VictoriaMetrics TSDB (15d) + VictoriaLogs storage (30d).
          # The victoriametrics/victorialogs module header comments
          # treat these as ephemeral *for host-failure DR*, but routine
          # reboots shouldn't wipe history — persist them so the
          # configured retention windows actually mean what they say.
          # Until #578 these entries still named the Prometheus/Loki
          # dirs that #126 retired, so the live stores were on the
          # rolled-back @root subvolume and every reboot reset history
          # to zero (which also re-armed every long-`for:` alert).
          #
          # Both units are DynamicUser + StateDirectory, so the real
          # storage is the /var/lib/private path and /var/lib/<name> is
          # only a symlink — same shape as the authentik and llama-cpp
          # entries, and bare strings for the same reason: ownership is
          # systemd's to manage, not ours. A preservation bindmount
          # hands the unit a root:root 0700 dir on a fresh boot; systemd
          # chowns an existing StateDirectory to the on-disk sentinel
          # (nobody:nogroup) before ExecStart and the unit writes
          # through the idmapped mount normally. No user/group/mode pin
          # (unlike /var/lib/mysql above, where tmpfiles fights us).
